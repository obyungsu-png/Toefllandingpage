...
export function ListeningM2Q15({ onBack, onNext, onHome, onVolumeClick }: ListeningM2Q15Props) {
  const [selectedAnswer, setSelectedAnswer] = useState<string | null>(null);

  const answerOptions = [
    "By disposing of waste appropriately",
    "By joining an environmental organization",
    "By using environmentally friendly forms of transportation",
    "By eating locally produced food"
  ];
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex gap-16 items-start justify-center pl-12 mt-12">
            ...
            <div className="flex-1 max-w-xl mt-8">
              <h2 className="font-['Inter',_sans-serif] font-bold text-gray-800 mb-6 text-lg">What does the speaker mention as a way that people can reduce their ecological footprint?</h2>
              <div className="space-y-6">
                {answerOptions.map((option, index) => (
                  <div key={index} className="flex items-start gap-4">
                    ...
                    <label htmlFor={`m2-q15-option-${index}`} className="font-['Inter',_sans-serif] text-gray-800 cursor-pointer leading-relaxed text-lg">
                      {option}
                    </label>
                  </div>
                ))}
