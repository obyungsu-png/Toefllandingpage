...
export function renderDailyLifePassage(passageText: string): React.ReactNode | null {
  ...
  switch (parsed.structure) {
      ...
      return (
        <div className="border-[3px] p-4 md:p-6" style={{ borderColor: c }}>
          <div className="border-2 p-4 md:p-6" style={{ borderColor: c }}>
            {f.title && <h2 className="text-xl md:text-2xl font-['Inter',_sans-serif] font-bold text-center mb-3 md:mb-4 text-gray-900 dark:text-gray-100">{f.title}</h2>}
            {f.subtitle && <p className="text-center font-['Inter',_sans-serif] font-medium mb-3 md:mb-4 text-gray-900 dark:text-gray-100">{f.subtitle}</p>}
            {f.body && <p className="font-['Inter',_sans-serif] leading-relaxed whitespace-pre-wrap text-sm md:text-base text-gray-900 dark:text-gray-100">{f.body}</p>}
          </div>
        </div>
      );
    ...
    case 'email':
      return (
        <div className="border-2 md:border-4 rounded-lg overflow-hidden bg-white dark:bg-gray-800" style={{ borderColor: c }}>
          <div className="bg-white dark:bg-gray-800">
            {['to', 'from', 'date', 'subject'].map(key => f[key] ? (
              <div key={key} className="flex border-b-2" style={{ borderColor: c }}>
                ...
                <div className="flex-1 bg-white dark:bg-gray-800 px-2 sm:px-4 py-1 sm:py-1.5 font-['Inter',_sans-serif] text-xs sm:text-sm text-gray-900 dark:text-gray-100">{f[key]}</div>
              </div>
            ) : null)}
          </div>
          <div className="p-3 sm:p-6 font-['Inter',_sans-serif] leading-relaxed text-sm sm:text-base whitespace-pre-wrap border-2 sm:border-4 m-1 sm:m-2 max-h-[400px] overflow-y-auto rounded text-gray-900 dark:text-gray-100" style={{ borderColor: c }}>
            {f.body}
          </div>
        </div>
      );
    ...
    case 'email_simple':
      return (
        <div className="rounded-lg overflow-hidden border-2 bg-white dark:bg-gray-800" style={{ borderColor: c }}>
          {f.subject && (
            <div className="px-3 sm:px-4 py-2 text-white font-['Inter',_sans-serif] font-bold text-sm sm:text-base" style={{ backgroundColor: c }}>
              {f.subject}
            </div>
          )}
          <div className="p-3 sm:p-6 font-['Inter',_sans-serif] leading-relaxed text-sm sm:text-base whitespace-pre-wrap max-h-[400px] overflow-y-auto text-gray-900 dark:text-gray-100">
            {f.body}
          </div>
        </div>
      );
      ...
      return (
        <div className="border-2 border-gray-300 dark:border-gray-600 rounded-xl overflow-hidden">
          <div className="bg-gray-100 dark:bg-gray-700 px-4 py-2 border-b border-gray-300 dark:border-gray-600">
            <span className="font-['Inter',_sans-serif] font-bold text-sm text-gray-800 dark:text-gray-100">{f.platform}</span>
          </div>
          <div className="p-4">
            <div className="flex items-center gap-2 mb-3">
              <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm" style={{ backgroundColor: c }}>
              ...
              <div>
                <p className="font-['Inter',_sans-serif] font-bold text-sm text-gray-900 dark:text-gray-100">{f.username}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">{f.timestamp}</p>
              </div>
            </div>
            <p className="font-['Inter',_sans-serif] leading-relaxed text-sm mb-3 whitespace-pre-wrap text-gray-900 dark:text-gray-100">{f.content}</p>
            <div className="flex gap-4 text-xs text-gray-500 dark:text-gray-400 border-t border-gray-200 dark:border-gray-700 pt-2">
              {f.likes && <span>♥ {f.likes}</span>}
              {f.comments && <span>💬 {f.comments}</span>}
              {f.shares && <span>↗ {f.shares}</span>}
            </div>
    ...
    case 'advertisement':
      return (
        <div className="border-4 border-double border-black dark:border-gray-300 p-4 md:p-6">
          <div className="text-center mb-4">
            <h2 className="text-xl md:text-2xl font-['Inter',_sans-serif] font-black tracking-wide text-gray-900 dark:text-gray-100">{f.headline}</h2>
            {f.business && <p className="font-['Inter',_sans-serif] font-bold text-base md:text-lg mt-1 text-gray-900 dark:text-gray-100">{f.business}</p>}
          </div>
          {f.offer && (
            <div className="bg-yellow-50 border-2 border-yellow-400 rounded-lg p-4 mb-4 text-center">
              <p className="font-['Inter',_sans-serif] font-bold text-base md:text-lg text-gray-900 dark:text-gray-100">{f.offer}</p>
            </div>
          )}
          {f.details && <div className="font-['Inter',_sans-serif] text-sm leading-relaxed whitespace-pre-wrap mb-4 text-gray-900 dark:text-gray-100">{f.details}</div>}
          <div className="border-t-2 border-black dark:border-gray-300 pt-3 text-center">
            {f.location && <p className="font-['Inter',_sans-serif] text-sm font-medium text-gray-900 dark:text-gray-100">{f.location}</p>}
            {f.contact && <p className="font-['Inter',_sans-serif] text-xs text-gray-600 dark:text-gray-400 mt-1">{f.contact}</p>}
          </div>
        </div>
      );
      ...
      return (
        <div className="border-2 border-gray-300 dark:border-gray-600 rounded-lg overflow-hidden">
          ...
          <div className="p-4 md:p-5">
            <h2 className="text-lg md:text-xl font-['Georgia',_serif] font-bold mb-2 text-gray-900 dark:text-gray-100">{f.headline}</h2>
            <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400 mb-4 border-b border-gray-200 dark:border-gray-700 pb-3">
              {f.date && <span>{f.date}</span>}
              {f.author && <><span>|</span><span>{f.author}</span></>}
            </div>
            <p className="font-['Georgia',_serif] leading-relaxed text-sm whitespace-pre-wrap text-gray-900 dark:text-gray-100">{f.body}</p>
          </div>
        </div>
      );
      ...
      return (
        <div className="border-2 border-gray-400 rounded-lg overflow-hidden">
          ...
          <div className="p-4">
                ...
                <thead>
                  <tr className="bg-gray-100 dark:bg-gray-700">
                    {f.tableHeaders.split(',').map((h, i) => (
                      <th key={i} className="border border-gray-300 dark:border-gray-600 px-3 py-2 text-xs font-['Inter',_sans-serif] font-bold text-left text-gray-900 dark:text-gray-100">{h.trim()}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {f.tableRows.split('\n').filter(r => r.trim()).map((row, i) => (
                    <tr key={i} className={i % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-gray-700'}>
                      {row.split(',').map((cell, j) => (
                        <td key={j} className="border border-gray-300 dark:border-gray-600 px-3 py-2 text-xs font-['Inter',_sans-serif] text-gray-900 dark:text-gray-100">{cell.trim()}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
            ...
            {f.footer && <div className="text-xs font-['Inter',_sans-serif] whitespace-pre-wrap text-gray-700 dark:text-gray-300 border-t border-gray-300 dark:border-gray-600 pt-3">{f.footer}</div>}
          </div>
        </div>
      );
        ...
        <div className="border-2 rounded-[24px] md:rounded-[28px] p-3 md:p-4 max-w-md mx-auto" style={{ borderColor: c }}>
          ...
          <div className="space-y-2 md:space-y-3 max-h-[420px] overflow-y-auto">
            {parseTextMessages(f.messages || '').map((msg, i) => (
              <div key={i} className={`flex ${msg.side === 'right' ? 'justify-end' : 'justify-start'}`}>
                <div
                  className={`max-w-[82%] rounded-2xl px-3 md:px-4 py-1.5 md:py-2 ${msg.side === 'right' ? 'bg-gray-100 dark:bg-gray-700' : ''}`}
                  style={msg.side === 'left' ? { backgroundColor: c + '18' } : {}}
                >
                  <p className="font-['Inter',_sans-serif] font-bold text-[11px] md:text-xs mb-0.5 md:mb-1" style={{ color: msg.side === 'left' ? c : '#374151' }}>
                    {msg.sender}{msg.time ? ` (${msg.time})` : ''}:
                  </p>
                  <p className="font-['Inter',_sans-serif] text-sm whitespace-pre-wrap text-gray-900 dark:text-gray-100">{msg.text}</p>
                </div>
              </div>
            ))}
          </div>
