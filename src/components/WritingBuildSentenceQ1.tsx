...
export function WritingBuildSentenceQ1({ 
  onBack, 
  onNext, 
  onHome, 
  onVolumeClick,
  avatar1ImageUrl,
  avatar2ImageUrl,
  questionText = 'What was the highlight of your trip?',
  words
}: WritingBuildSentenceQ1Props) {
  const defaultWords = ['were', 'the', 'was', 'old city', 'showed us around', 'who', 'tour guides'];
  ...
  // Calculate dynamic width based on word length - more natural sizing
  const getSlotWidth = (word: string | null) => {
    if (!word) return '120px';
    // Natural width calculation: base + character width
    const baseWidth = 30;
    const charWidth = 11;
                ...
                <div className="flex flex-wrap items-end gap-2">
                  {sentenceSlots.map((word, index) => (
                    ...
                    >
                      ...
                      {/* Individual underline for each slot */}
                      <div 
                        className="absolute bottom-0 left-0 right-0 border-b-2 border-gray-800"
                      ></div>
                    </div>
                  ))}
