...
export function TPOPage({
  ...
  const allTestNumbers = useMemo(() => {
    // Dynamically generate test numbers based on actual TPO tests data
    const maxTestNumber = tpoTests.reduce((max, test) => {
      if (test.testType === 'TPO' && test.testNumber > max) {
  ...
  const getFilteredTestNumbers = (): number[] => {
    ...
    // If year/month/official filters are active, filter based on tpoTests metadata
