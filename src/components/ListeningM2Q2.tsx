...
export function ListeningM2Q2({ onBack, onNext, onHome, onVolumeClick }: ListeningM2Q2Props) {
  const [selectedAnswer, setSelectedAnswer] = useState<string | null>(null);

  const answerOptions = [
    "Please wait while I look that up.",
    "Give me some dates.",
    "No, I have another due date.",
    "Yes, that's correct."
  ];
          ...
          <div className="flex gap-16 items-start justify-center pl-12 mt-12">
            ...
            <div className="flex-shrink-0">
              <div className="w-96 h-96 bg-white rounded-lg overflow-hidden">
                <ImageWithFallback 
                  src="figma:asset/c34029af9a04f96b0e53d6c821e6957a3fac4ad7.png" 
                  alt="Man in green polo shirt"
              ...
              <div className="space-y-6">
                {answerOptions.map((option, index) => (
                  <div key={index} className="flex items-start gap-4">
                    ...
                    <label htmlFor={`m2-q2-option-${index}`} className="font-['Inter',_sans-serif] text-gray-800 cursor-pointer leading-relaxed text-lg">
                      {option}
                    </label>
                  </div>
                ))}
