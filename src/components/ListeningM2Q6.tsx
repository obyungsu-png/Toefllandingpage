...
export function ListeningM2Q6({ onBack, onNext, onHome, onVolumeClick }: ListeningM2Q6Props) {
  ...
  const answerOptions = [
    "I'm revising my resume.",
    "I'll set up a meeting for us to talk.",
              ...
              <div className="space-y-6">
                {answerOptions.map((option, index) => (
                  <div key={index} className="flex items-start gap-4">
                    ...
                    <label htmlFor={`m2-q6-option-${index}`} className="font-['Inter',_sans-serif] text-gray-800 cursor-pointer leading-relaxed text-lg">
                      {option}
                    </label>
                  </div>
                ))}
