...
import speakingImage from 'figma:asset/687ad575c6c75acb823f8d7c22b5606648040912.png';
...
interface SpeakingQ7RecordProps {
  onNext: () => void;
  onHome: () => void;
  onVolumeClick?: () => void;
...
export function SpeakingQ7Record({ onNext, onHome, onVolumeClick, isVolumeOpen, volumeButtonRef, imageUrl, questionText, responseDelay, stopDuration }: SpeakingQ7RecordProps) {
  ...
  useEffect(() => {
    if (showStopOverlay && recorder.audioBlob && !uploadedRef.current) {
      ...
      uploadRecording(recorder.audioBlob, 7).catch(() => {});
    }
