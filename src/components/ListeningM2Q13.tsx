...
export function ListeningM2Q13({ onBack, onNext, onHome, onVolumeClick }: ListeningM2Q13Props) {
  const [selectedAnswer, setSelectedAnswer] = useState<string | null>(null);

  const answerOptions = [
    "Changes in consumption in societies over time",
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex gap-16 items-start justify-center pl-12 mt-12">
            ...
            <div className="flex-1 max-w-xl mt-8">
              <h2 className="font-['Inter',_sans-serif] font-bold text-gray-800 mb-6 text-lg">What is the main topic of the talk?</h2>
              <div className="space-y-6">
                {answerOptions.map((option, index) => (
                  <div key={index} className="flex items-start gap-4">
                    ...
                    <label htmlFor={`m2-q13-option-${index}`} className="font-['Inter',_sans-serif] text-gray-800 cursor-pointer leading-relaxed text-lg">
                      {option}
                    </label>
                  </div>
                ))}
