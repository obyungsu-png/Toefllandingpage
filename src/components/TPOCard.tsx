...
export function TPOCard({ number, displayNumber, onStartTest, onReviewTest, onContinueTest, isLocked = false, onUnlockClick, testData, customTitle, bankType = 'tpo' }: TPOCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="bg-gradient-to-r from-[#2d7a7c] to-[#3d8a8c] rounded-t-[12px] shadow-md flex items-center justify-between px-4 py-3 min-h-[52px]">
        <p className="font-['Inter',_sans-serif] text-white text-xl sm:text-2xl font-bold tracking-wide">{customTitle || `TPO ${displayNumber ?? number}`}</p>
